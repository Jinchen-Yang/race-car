...
/* ===== 每圈计数(✅ 已实测, 2026-07-03 十圈法, 手转 10 圈取平均) =====
 * 左轮: 计数 116 -> 10290, (10290-116)/10 = 1017.4 计数/圈  [QEI 4 倍频解码]
 * 右轮: 计数增量 2652/10 = 265.2 计数/圈                    [单相上升沿 1 倍频]
 * 左右不同是"解码倍频不同"所致(同款电机/编码器), 属正常, 勿"统一"改成一样。
 * ⚠ 换电机、换编码器、或改 QEI/中断解码方式后必须重测。 */
#define ENC_L_CNT_PER_REV  1017.4f  /**< 左轮每圈计数(实测) */
#define ENC_R_CNT_PER_REV   265.2f  /**< 右轮每圈计数(实测) */
