...
#define BASE_SPEED        170    /* ⚠r25 语义变更: 巡航"占空"(不再是 mm/s)! ≈300mm/s.
                                  * 'V/v' ±10 在线调, 夹 [60,300] */
 ...
 * 作用域(r40 修正, 见 GRAY_LOOKAHEAD_MM): 弧段内, 认到线时全额生效; 丢线后按"车体走完
 *   传感器前置量所需的帧数"继续保持。
 *   r39 初版把保持窗拍成 3 帧, 依据是"C 处切线即 C→D 方向, 所以线一没就该直走"。这句话
 *   对【传感器】成立, 对【车体】不成立 —— 线是传感器丢的, 而那一刻车体还在 C 点后方
 *   GRAY_LOOKAHEAD_MM 处、仍骑在弧上, 必须再画 L/R 弧度才走到 C。L=110/R=400 即 15.8°。
 *   只托 3 帧(2.9°)的后果: 剩下约 13° 被甩给 seg2 的绝对航向保持, 它用 TRACK_KP=6.0 一把
 *   拉回 => diff≈-78 => "先直冲出去、再猛地向右一甩"(2026-07-09 真机实测症状)。
 *   保持窗的天然上限由段状态机给: 连续空白 SEG_BLANK_TH=16 帧后 g_seg 前进, on_arc 转假,
 *   前馈自灭 —— 所以窗开大了也不会把弧无限画下去。
...
/* r40: 传感器前置量 —— 灰度阵列中心到【驱动轮轴】的纵向距离(mm)。
 * 它是"丢线后前馈还要托多久"的唯一物理依据: 线是传感器丢的, 车体还落后 L, 仍在弧上。
 * 保持帧数 = L / 每帧行程 = L / (v·dt), v = g_base_speed / DUTY_PER_MMS, dt = APP_TRACK_DT_MS。
 *   @base=170: v=340mm/s, 每帧 6.8mm, L=110 -> 16 帧(≈320ms) —— 恰好与 SEG_BLANK_TH 的
 *   出弧检测窗同量级, 因为两者量的是同一段路程。
 * 用帧数而非硬编码常数的好处: 'V/v' 在线调速时保持窗自动跟随(基速越高, 同样的领先量占的
 * 帧数越少); 硬编码一个帧数就会在改速后悄悄失配。
 * ⚠ 110.0f 是估值! 实测法: 尺子量灰度探头一排的中心到两个驱动轮连线的垂直距离。
 * 验证判据: CSV 里 seg 从 1 跳到 2 的那一拍, dy 应接近 0。
 *   dy 强负(≈-10~-20°) = 欠转, L 估小了(前馈撤太早, 车没转够, 航向环随后猛拉右);
 *   dy 强正            = 过转, L 估大了(前馈托太久, 弧画过头)。
 * ⚠低速下补偿会被截断: 保持窗的上限是"连续空白 SEG_BLANK_TH=16 帧"这个【帧数】门限, 而车体
 *   走完 L 需要的是一段【距离】。base=170 时 16 帧恰好 =109mm≈L, 对得上; base 降到 150/100/60
...
#define SEG_LINE_TH       0x08   /* 漏积分 ≤此 = 确认在线(≈5个有线帧)。
                                  * ⚠r31 从 63 回退: 63 实为 2 帧即认(255→127→63, 我当时算错说3帧),
                                  * 白布褶皱/阴影连续2帧就误触, 段状态机被假事件链式推进(0→1→2,
                                  * 目标航向翻转)= "走走停停/巡线乱/F1崩"的元凶(2026-07-06 真机实锤)。
                                  * 旧病"正着冲线偶尔认不出"随之回归(偶发, 可忍), 根治待"触线降速
                                  * 确认"方案(CTY原版做法), 另行报批。 */
#define SEG_BLANK_TH      0x10   /* 漏积分 ≥此 = 确认空白(16空白帧≈320ms) */
#define GRAY_MIN_HITS_DEFAULT 2  /* 抗阴影门槛: 至少几路同时压黑才算真线。
                                  * 1=最灵敏(旧行为), 2=默认滤掉单路阴影, 3=更抗阴影但可能漏窄线。
                                  * 串口 G/g 在线调, CSV 看 gray_hits/line_ok。 */
#define START_PROTECT_MS  1500   /* 起步保护: 强制seg0+无视灰度(A点残端免疫)。
                                  * r31 随认线门限一并回退到 1500(恢复 r28 已验证组合);
                                  * ⚠此窗内撞线必然直穿, 摆车离胶带 ≥50cm 起步。 */
...
#define DRIVE_TRIM_DUTY_R   5     /* 待整定: 右轮占空偏置默认值(现场调好后回填) */
 ...
 * 本车物理接线与 PAN/TILT 命名相反(见 r37 实测):
 *     水平 360° 连续舵机 接在 SERVO_TILT/PA16 ('K' 键那路, 一直转);
 *     垂直 180° 位置舵机 接在 SERVO_PAN /PA17 ('J' 键那路, 能定角)。
 * 方案: 进 AIM 时连续舵机朝靶侧慢转做"转过去瞄准"手势, 再发停转脉宽保持;
 * 垂直位置舵机斜坡抬到俯仰角定激光高度。恒返回 0 让 AIM 满 5s(激光亮足, 录像清晰)。
 * 连续舵机开环无位置反馈: 单向手势每趟停点会累积漂移, 靶按"停稳后激光落点"摆。
...
#define AIM_ELEV_DEG       105   /* 垂直俯仰目标角(送 AIM_ELEV_CH): r37 实测激光对齐靶心 */
#define AIM_ELEV_RAMP_DEG  1.0f  /* 俯仰斜坡步进(°/10ms拍): 防满速甩头 */
#define AIM_CONT_STOP_US   1495u /* 连续舵机停转脉宽: r37 实测 1495us 才真不转 */
...
static uint8_t  g_gray_min_hits = GRAY_MIN_HITS_DEFAULT; /**< 认线最小黑点数('G/g' 调, 1..4) */
static uint8_t  g_gray_hits_dbg = 0;    /**< 最近一拍灰度黑点数(0..8), 给 CSV 排障 */
static uint8_t  g_gray_line_ok_dbg = 0; /**< 最近一拍是否通过抗阴影门控 */
...
static uint8_t gray_hit_count_from_byte(uint8_t b)
{
    /* 感为数字字节: bit=0 表示该路压黑线, 只统计低 8 位里的 0。 */
    uint8_t inv = (uint8_t)~b;
    uint8_t n = 0;
    for (uint8_t i = 0; i < 8u; i++) {
        if (inv & (uint8_t)(1u << i)) n++;
    }
    return n;
}
...
{
    uint8_t h = fresh ? gray_hit_count_from_byte(byte) : 0u;
    uint8_t ok = (uint8_t)(fresh && h >= g_gray_min_hits && h < GRAY_CH_NUM);
    if (hits)     *hits = h;
    if (line_ok)  *line_ok = ok;
    if (min_hits) *min_hits = g_gray_min_hits;
}
...
void track_loop_step(void)
{
    ...
    app_eval_gray_gate(byte, fresh, &g_gray_hits_dbg, &g_gray_line_ok_dbg, 0);

    /* 2) CTY 漏积分丢线计数: 空白帧自增(封顶), 有线帧减半 —— 快认线/慢认空。
     * r32: I2C 失败帧显式当"无数据"——不推进计数、不喂查表(审计定案: 失败时
     * 旧字节被冻结, 旧字节含线=朝固定方向拐死, 旧字节全白=段误切/掉头)。 */
    if (fresh) {
        if (g_gray_line_ok_dbg && g_nodata > 0u) {
            g_nodata >>= 1;
        } else if (g_nodata < 0xFF) {
            g_nodata++;
        }
    }
    ...
    if (g_seg == 0u) {
         ...
         * 质心范围约 ±350, 乘 g_arc_err_to_deg 换算成等效角度偏差。
         * gray_get_error 约定线在左=负; 控制接口 dy>0=左转, 所以这里必须取反。 */
         ...
         * 转向交给曲率前馈, 托到"车体走完传感器前置量"为止(见 GRAY_LOOKAHEAD_MM):
         * 线是传感器丢的, 车体还落后 L 仍在弧上, 得把这段弧画完才到几何末端。
         * 之后前馈撤掉 = 沿切线直走; 段状态机再攒够 SEG_BLANK_TH 帧就正式出弧。 */
        dy = 0.0f;
        dy_src = 3u;
    }

    /* 7) 曲率前馈作用域: 弧段内, 认到线时全额生效; 丢线后再保持"车体走完传感器前置量 L"
     * 所需的帧数(见 GRAY_LOOKAHEAD_MM)。这同时覆盖了灰度闪断 —— 否则前馈被闪断稀释成
     * 欠转, 线更往弯道外跑 -> 更易丢 -> 正反馈, 还在闪断频率上给电机一个 ±ff 的方波。
     * 保持窗天然被段状态机截断: 连续空白 SEG_BLANK_TH 帧后 g_seg 前进, on_arc 转假。 */
    ...
    /* 保持帧数 = L / 每帧行程; 每帧行程 = (base / DUTY_PER_MMS) mm/s × APP_TRACK_DT_MS ms。
     * g_base_speed 夹在 [60,300] => 每帧 2.4~12mm => 帧数 10~46, 不会溢出 uint8_t。 */
    float mm_per_frame = ((float)g_base_speed / DUTY_PER_MMS)
                       * ((float)APP_TRACK_DT_MS / 1000.0f);
    uint8_t ff_hold = (mm_per_frame > 0.1f)
                    ? (uint8_t)(GRAY_LOOKAHEAD_MM / mm_per_frame + 0.5f)
                    : (uint8_t)SEG_BLANK_TH;   /* 兜底: 基速异常小时按出弧窗长度 */
    uint8_t ff_on = (uint8_t)(on_arc && s_arc_lost <= ff_hold);
...
void app_fsm_step(void)
{
            ...
            while (g_node_pending > 0u) {
                ...
                if (g_run_mode == 3u && g_kp_idx == 1u) {
                    ...
                    if (g_run_mode == 4u && (g_lap_count + 1u) < APP_LAPS_MODE4) {
                        ...
                        g_kp_idx = 0;
                        /* r29 限幅重锚(修 r27 设计失误): 出线姿态噪声可达 ±15°, r27 的
                         * "±30°门限内整锚替换"把噪声一次性注入下一圈(lap2 起步右偏实锤,
                         * 药比病毒——漂移每圈才≈7°)。改为"旧 z + 夹±10°的观测修正":
                         * 漂移吃得下, 出线噪声最多污染 10° 且下一圈还会被再修, 逐圈收敛。 */
                        {
                            float derr = wrap180(imu_get_yaw() - g_datum_yaw);
                            derr = clampf(derr, -10.0f, 10.0f);
                            g_datum_yaw = g_datum_yaw + derr;
                        }
                    } else {
    ...
    case APP_ST_AIM:
        ...
        motor_stop_all();
        {
            ...
            if (entered) {
                /* 先写好目标 PWM 再开舵机轨, 避免上电瞬间按旧角度大幅扫动。 */
                servo_rail_enable(1);
            }
...
void app_estop_ack(void)
{
    ...
    if (s_cnt >= 3u) {
        s_cnt = 0;
        app_return_idle();          /* 清障回待机; IDLE 态会保持电机停 */
    }
}
...
void app_return_idle(void)
{
    ...
    led_run_set(0);
    led_err_set(0);
    app_init();     /* 保留 g_run_mode 与在线调参副本, 只清运行态/进度/PID 历史 */
}

void app_tune_step(char which, int dir)
{
    /* 串口在线调参(现场整定免重编译): 单字符增减, 下一拍外环立即生效。 */
    ...
    if (which == 'p') {
        ...
        g_arc_err_to_deg += 0.01f * d;                 /* 灰度质心->角度比例, 过弯太冲/左右晃可先减 */
        if (g_arc_err_to_deg < 0.00f) g_arc_err_to_deg = 0.00f;
        if (g_arc_err_to_deg > 0.30f) g_arc_err_to_deg = 0.30f;
    } else if (which == 'v') {
        ...
        /* 灰度抗阴影门槛: G 增大=更不敏感, g 减小=更容易认线。 */
        int v = (int)g_gray_min_hits + ((dir >= 0) ? 1 : -1);
        if (v < 1) v = 1;
        if (v > 4) v = 4;
        g_gray_min_hits = (uint8_t)v;
    }
}
...
{
    /* 现场微调水平 360° 连续舵机的"停转脉宽"到纹丝不动, 立即写入通道可看效果。
     * 本版 G/g 已用于灰度抗阴影, 这里改用 Y/y: Y +5us, y -5us。 */
    if (dir > 0 && g_cont_stop_us < 1700u) {
        g_cont_stop_us += 5u;
    } else if (dir < 0 && g_cont_stop_us > 1300u) {
        g_cont_stop_us -= 5u;
    }
    servo_set_pulse_us(AIM_CONT_CH, g_cont_stop_us);
}
...
{
    return g_cont_stop_us;   /* 供 'Y/y' 回显; 调好写死进 AIM_CONT_STOP_US */
}
...
{
    return g_arc_err_to_deg;
}

void app_get_gray_gate_debug(uint8_t *hits, uint8_t *line_ok, uint8_t *min_hits)
{
    if (hits)     *hits = g_gray_hits_dbg;
    if (line_ok)  *line_ok = g_gray_line_ok_dbg;
    if (min_hits) *min_hits = g_gray_min_hits;
}
...
{
    if (seg)          *seg = g_seg;
    if (nodata)       *nodata = g_nodata;
    if (protect_ms)   *protect_ms = g_protect_ms;
    if (startprot_ms) *startprot_ms = g_startprot_ms;
}
